...
get_latest_release() {
  latest_release=$(curl -s https://api.github.com/repos/ArmaanLala/linutil/releases | 
    grep "tag_name" | 
    head -n 1 | 
    sed -E 's/.*"tag_name": "([^"]+)".*/\1/')
...
redirect_to_latest_pre_release() {
  ...
  if [ -n "$latest_release" ]; then
    url="https://github.com/ArmaanLala/linutil/releases/download/$latest_release/linutil"
  else
    printf "%b\n" 'Unable to determine latest pre-release version.' >&2
    printf "%b\n" "Using latest Full Release"
    url="https://github.com/ArmaanLala/linutil/releases/latest/download/linutil"
  fi
  addArch
  printf "%b\n" "Using URL: $url"
}
